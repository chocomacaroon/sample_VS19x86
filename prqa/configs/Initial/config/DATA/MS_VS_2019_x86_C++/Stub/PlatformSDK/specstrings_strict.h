...
#define __allowed(p) /* nothing */
#else
#define __allowed(p) __allowed_##p
#define __allowed_as_global_decl /* empty */
#define __allowed_as_statement_with_arg(x) \
    __pragma(warning(push)) __pragma(warning(disable : 4548)) \
        do {__noop(x);} while((0,0) __pragma(warning(pop)) )
#define __allowed_as_statement __allowed_as_statement_with_arg(1)

/**************************************************************************
*  This should go away. It's only for __success which we should split into.
*  __success and __typdecl_sucess
***************************************************************************/
#define __allowed_on_function_or_typedecl /* empty */
#if (__SPECSTRINGS_STRICT_LEVEL == 1) || (__SPECSTRINGS_STRICT_LEVEL == 2)
#define __allowed_on_typedecl /* empty */
#define __allowed_on_return /* empty */
#define __allowed_on_parameter /* empty */
#define __allowed_on_function /* empty */
#define __allowed_on_struct /* empty */
#define __allowed_on_field /* empty */
#define __allowed_on_parameter_or_return /* empty */
#define __allowed_on_global_or_field /* empty */
#elif __SPECSTRINGS_STRICT_LEVEL == 3
#define __allowed_on_typedecl /* empty */
/* Define dummy source attributes. Still needs more testing */
#define __allowed_on_return [returnvalue: OnReturnOnly]
#define __allowed_on_parameter [OnParameterOnly]
#define __allowed_on_function [method: OnFunctionOnly]
#define __allowed_on_struct [OnStructOnly]
#define __allowed_on_field [OnFieldOnly]
#define __allowed_on_parameter_or_return [OnParameterOrReturnOnly] 
#define __allowed_on_global_or_field /* empty */
#pragma push_macro( "DECL_SA" )
#pragma push_macro( "SA" )
#ifdef __cplusplus
